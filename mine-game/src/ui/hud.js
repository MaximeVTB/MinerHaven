...
import { GEAR } from '../data/gear.js?v=20260827-2';
import { LAYERS, LAYERS_BY_ID } from '../data/layers.js';
import { PICKAXES, PICKAXES_BY_ID } from '../data/pickaxes.js?v=20260827-2';
...
function recipeRows(state, resultType) {
  return RECIPES.filter((recipe) => recipe.resultType === resultType).map((recipe) => {
    ...
    return `<article class="operation-row">
      <div>${itemTitle(result)}<span>${result.description}</span><small class="mono">${costText(recipe, state)}</small></div>
...
function autoMiningSpeedRecipeRow(state) {
  ...
  return `<article class="operation-row">
    ...
    <button class="btn" data-action="craft" data-id="${nextRecipe.id}" ${canCraft ? '' : 'disabled'}>craft</button>
  </article>`;
}
...
export function renderWorkshop(state, panel) {
  ...
  if (panel === 'loadout') {
    ...
    content.innerHTML = `${menuBack}<p class="panel-intro">Your active tool controls the mining pattern. Gear stacks across different slots.</p>
      <h3>Pickaxes</h3>${pickaxes.map((pickaxe) => `<article class="operation-row"><div>${itemTitle(pickaxe)}<span>${pickaxe.description}</span></div><button class="btn" data-action="equip-pickaxe" data-id="${pickaxe.id}" ${state.currentPickaxeId === pickaxe.id ? 'disabled' : ''}>${state.currentPickaxeId === pickaxe.id ? 'equipped' : 'equip'}</button></article>`).join('')}
      <h3>Gear</h3>${gear.length ? gear.map((item) => `<article class="operation-row"><div>${itemTitle(item)}<span>${item.description}</span><small>${item.slot} slot</small></div><button class="btn" data-action="equip-gear" data-id="${item.id}" ${state.equippedGearBySlot[item.slot] === item.id ? 'disabled' : ''}>${state.equippedGearBySlot[item.slot] === item.id ? 'equipped' : 'equip'}</button></article>`).join('') : '<p class="empty-state">Craft gear at the Gear Shop.</p>'}`;
    return;
  }
