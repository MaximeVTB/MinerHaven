...
function getPowerGearCells(state, cx, cy, grid) {
  const powerGearId = state.equippedGearBySlot.power;
  const powerGear = GEAR_BY_ID[powerGearId];
  if (!powerGear || state.rng() >= powerGear.procChance) return [];

  if (powerGear.blockEffect === 'adjacent') {
    const directions = [[0, -1], [1, 0], [0, 1], [-1, 0]];
    const [dx, dy] = directions[Math.floor(state.rng() * directions.length)];
    return [[cx + dx, cy + dy]].filter(([x, y]) => inBounds(grid, x, y));
  }
  if (powerGear.blockEffect === 'burst') {
    const cells = [];
...
export function mineBlock(state, cx, cy, pickaxeId = state.currentPickaxeId, applyPowerGearEffect = pickaxeId === state.currentPickaxeId) {
  ...
  const powerGearCells = applyPowerGearEffect ? getPowerGearCells(state, cx, cy, state.grid) : [];
  const cells = [...new Map([...pickaxeCells, ...powerGearCells].map(([x, y]) => [`${x},${y}`, [x, y]])).values()];
  const results = [];
